...
9. 表記ゆれの検出・用語集の作成は [references/glossary.md](references/glossary.md) の原則に従う。`python "${CLAUDE_PLUGIN_ROOT}/scripts/jp_glossary.py" variants|terms <file|->` で候補を機械的に抽出できるが、確実に検出できるのはカタカナ長音・全角半角のゆれのみで、送り仮名等の候補は精度が低いため必ずClaudeが判断を補う(`/glossary` コマンド参照)。
