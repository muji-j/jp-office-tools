...
2. **軽い経路(既定)**: 自然文・貼付内容から構成パターン([jp-bizdoc/references/slides.md](../skills/jp-bizdoc/references/slides.md) の結論先行型/経緯報告型/比較検討型)を推定し、[jp-slides/SKILL.md](../skills/jp-slides/SKILL.md) のコンテンツ JSON スキーマに沿って内容を組み立てる。テーマ(16種、それぞれ固有のモダンデザイン)は用途(社内向け/対外提案/夜間投影 等)から妥当なものを推定する。KPI・指標を数字で見せたい場合は `stats` スライド(`items[]`: value/label/note)を使う。用途・想定読み手・テーマの方向性が不明瞭な場合のみ最小限確認し、明白なら確認を省いて初稿を作ってから調整を提案する。
